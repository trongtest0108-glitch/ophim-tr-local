...
function crawl_ophim_movies()
{
    ...
    $filterYear = oIsset($_POST, 'filterYear', []);

    $result = crawl_ophim_movies_handle($url, $ophim_id, $ophim_update_time, $filterType, $filterCategory, $filterCountry, $filterYear);
    echo $result;
    die();
}

function crawl_ophim_movies_handle($url, $ophim_id, $ophim_update_time, $filterType, $filterCategory, $filterCountry, $filterYear = [])
{
...
function create_data($sourcePage, $url, $ophim_id, $ophim_update_time, $filterType = [], $filterCategory = [], $filterCountry = [], $filterYear = [])
{
    ...
    $movieYear = (string)oIsset($sourcePage["movie"], "year", '');
    $filterYear = array_map('strval', $filterYear);
    if ($movieYear !== '' && in_array($movieYear, $filterYear, true)) {
        return array(
            'crawl_filter' => true,
        );
    }
